...
function fusionPrivacyBar() {
	...
	jQuery( '.fusion-privacy-placeholder' ).each( function() {
		...
		if ( $iframe.is( 'iframe' ) && ! $parent.hasClass( 'fusion-background-video-wrapper' ) ) {
	...
	jQuery( '.fusion-privacy-bar-learn-more' ).on( 'click', function( event ) {
		...
		// Limit the scroll event to the .fusion-privacy-bar container when hovering it with the mouse or touching, i.e. exclude body scroll.
		setTimeout( function() {
			if ( privacyBarWrapper.hasClass( 'fusion-privacy-bar-open' ) && privacyBarWrapper.outerHeight() >= jQuery( window ).height() ) {
				privacyBarWrapper.limitScrollToContainer();
			} else {
				privacyBarWrapper.unbind( 'mousewheel DOMMouseScroll touchmove' );
			}
		}, 300 );

		// Switch angle direction.
		if ( jQuery( this ).find( '.fusion-icon-angle-up' ).length ) {
			jQuery( this ).find( '.fusion-icon-angle-up' ).removeClass( 'fusion-icon-angle-up' ).addClass( 'fusion-icon-angle-down' );
		} else if ( jQuery( this ).find( '.fusion-icon-angle-down' ).length ) {
			jQuery( this ).find( '.fusion-icon-angle-down' ).removeClass( 'fusion-icon-angle-down' ).addClass( 'fusion-icon-angle-up' );
		}
	} );
