/* global avadaMenuVars, Modernizr, avadaHeaderVars */
/* eslint max-depth: 0 */
jQuery( document ).ready( function() {
	...
	if ( jQuery( '.fusion-is-sticky' ).length && jQuery( '.fusion-mobile-sticky-nav-holder' ).length ) {
		jQuery( '.fusion-mobile-menu-icons .fusion-icon-bars' ).attr( 'aria-controls', jQuery( '.fusion-mobile-sticky-nav-holder > ul' ).attr( 'id' ) );
	} else {
		jQuery( '.fusion-mobile-menu-icons .fusion-icon-bars' ).attr( 'aria-controls', jQuery( '.fusion-mobile-nav-holder' ).not( '.fusion-mobile-sticky-nav-holder' ).find( '> ul' ).attr( 'id' ) );
	}
	...
	jQuery( document ).on( 'click', '.fusion-mobile-menu-icons .fusion-icon-bars', function( event ) {
	...
	jQuery( document ).on( 'click', '.fusion-mobile-menu-icons .fusion-icon-search', function( event ) {
		event.preventDefault();
	...
	function fusionMobileMenu() {
		...
		if ( avadaMenuVars.submenu_slideout && 'flyout' !== avadaMenuVars.mobile_menu_design ) {
			...
			jQuery( '.fusion-mobile-nav-holder a:not(.fusion-menu-edit, .fusion-icon-sliding-bar)' ).click( function( e ) {
		...
		if ( $flyoutContent.hasClass( 'fusion-flyout-active' ) ) {
			if ( $flyoutContent.hasClass( 'fusion-flyout-menu-active' ) ) {
				...
				jQuery( document ).on( 'keyup.flyoutSearchEsc', function( event ) {
					if ( 27 === event.keyCode ) { // Escape key.
						document.querySelector( '.fusion-flyout-menu-icons .fusion-icon.fusion-icon-search' ).click();
					}
				} );
			} else {
				$flyoutContent.removeClass( 'fusion-flyout-active' );
...
jQuery( window ).on( 'load', function() {
	...
	jQuery( '.fusion-menu-login-box' ).each( function() {
		...
		loginBox.find( 'input[id="username"]' ).click( function() {
		...
		loginBox.find( 'input' ).not( '[id="username"]' ).click( function() {
