...
!function () {
	...
	function rewriteCss(css) {
		...
		*/
		...
		return css.replace(regFindSetters, function($0, $1, $2, $3, important){
			return $1+'-ie-'+(important?'â—':'')+$3;
		}).replace(regFindGetters, function($0, $1, $2, important){
			return $1+'-ieVar-'+(important?'â—':'')+$2+'; '+$2; // keep the original, so chaining works "--x:var(--y)"
		});
	}
	...
	function parseRewrittenStyle(style) { // less memory then parameter cssText?
		...
		if (matchesGetters) {
			...
			for (j = 0; match = matchesGetters[j++];) {
				...
				if (propName[0] === 'â—') propName = propName.substr(1);
	...
	function _drawElement(el) {
		...
		for (var prop in el.ieCPSelectors) {
			var important = style['-ieVar-â—' + prop];
	...
	StyleProto.getPropertyValue = function (property) {
		...
		const iePropertyImportant = '-ie-â—'+undashed;
	...
	StyleProto.setProperty = function (property, value, prio) {
		...
		property = '-ie-'+(prio==='important'?'â—':'') + property.substr(2);
