	...
	iLightBox.prototype = {
		...
		normalizeItems: function() {
			...
			$.each(iL.items, function(key, val) {
				...
				// ThemeFusion edit for Youtube and Vimeo thumbnails.
				if ( 'iframe' === type ) {
					if ( -1 !== URL.indexOf( 'youtube.com' ) ) {
						...
						if ( $match && 11 === $match[7].length ) {
							options.thumbnail = '//img.youtube.com/vi/'+ $match[7] +'/mqdefault.jpg';
						}
					} else if( -1 !== URL.indexOf( 'vimeo.com' ) ) {
						$videoID = URL.split(/[?#]/)[0].replace ( /[^\d]/g, '' );
						$.getJSON( 'https://www.vimeo.com/api/v2/video/' + $videoID + '.json?callback=?', {format: "json"}, function( data ) {
							options.thumbnail = data[0].thumbnail_medium;
						});
					}
				}
				// ThemeFusion edit ends.

				if (!options.width || !options.height) {
					// ThemeFusion edit for Avada theme; set width from 1280x700 (100% for iframe) to the theme option values
					if ( 'video' === type || 'iframe' === type || 'flash' === type ) {
						options.width  = parseInt( fusionLightboxVideoVars.lightbox_video_width );
						options.height = parseInt( fusionLightboxVideoVars.lightbox_video_height );
					}
				}
		...
		positionThumbnails: function(thumbnails, container, grid) {
			...
			thumbs.each(function(i) {
				...
				(path == 'horizontal') ? (
					gridWidth += t.outerWidth( true ) // ThemeFusion edit:: include margin.
				) : (
		...
		patchItemsEvents: function() {
			var iL = this,
				vars = iL.vars,
				clickEvent = supportTouch ? "click.iL itap.iL" : "click.iL", // ThemeFusion edit, see #6258.
				vEvent = supportTouch ? "click.iL itap.iL" : "itap.iL"; // ThemeFusion edit, see #6258.
		...
		patchEvents: function() {
			...
			$win.bind('resize.iLightBox', function() {
				...
				// ThemeFusion edit to prevent resize while in transition.
			...
			$doc.on(clickEvent, '.ilightbox-overlay', function() {
				...
				// ThemeFusion edit for Avada theme: Fixes 7122.
				if (t.hasClass('fusion-updated')) return;
		...
		moveTo: function(side) {
			var iL = this,
				...
				// ThemeFusion edit for Avada theme: Fixs 6080.
				currentElement = iL.vars.holder,
				iframeURL = ( currentElement.find( 'iframe' ).length ) ? currentElement.find( 'iframe' ).attr( 'src' ) : '';

			if ( iframeURL && -1 !== iframeURL.indexOf( 'vimeo.com' ) ) {
				currentElement.find( 'iframe' ).attr( 'src' , iframeURL );
			}
			// ThemeFusion edit ends.
			...
			if (vars.nextLock || vars.prevLock) return false;
				...
				// ThemeFusion edit for Avada theme: add item to conditional to fix issue with single images.
				if (opts.linkId && item ) {
					vars.hashLock = true;
		...
		closeAction: function() {
			...
			// ThemeFusion edit for Avada theme: sincethe iLightBox namespace is removed from click event we have to unregister events manually.
			...
			// ThemeFusion edit for Avada theme: make sure that the navigation arrows are reset when lightbox is closed.
			vars.prevButton.removeClass( 'disabled' );
			vars.nextButton.removeClass( 'disabled' );
			// End edit.
				...
				case 'iframe':
					...
					if ( obj.URL.indexOf( 'vimeo.com' ) != -1 ) {
						...
						if ( $match ) {
							...
							$query_string = '?title=0&amp;byline=0&amp;portrait=0';
							for ( var $i = 0; $i < $query_array.length; ++$i ) {
								var $single_query = $query_array[$i].split( '=' );
								if ( 1 < $single_query[0].length && 2 === $single_query.length && -1 === $query_string.indexOf( $single_query[0] )  ) {
									$query_string += '&' + $single_query[0] + '=' + decodeURIComponent( $single_query[1].replace( /\+/g, " " ) );
								}
							}

							var $movie = '//player.vimeo.com/video/'+ $match[3] + $query_string;
						} else {
							var $movie = obj.URL;
						}
                    ...
                    var allow = '';
                    if ( -1 !== $query_string.indexOf( 'autoplay=1' ) ) {
                        allow += 'autoplay'
                    }
			...
			if (index) {
				...
				if (iL.items[target]) {
					...
					if (overlay.length && overlay.attr('linkid') == opts.linkId) {
						...
						// ThemeFusion edit, see #6258.
						iL.itemsObject[target].trigger(supportTouch ? 'click itap' : 'click');
					}
				} else {
					var overlay = $('.ilightbox-overlay');
