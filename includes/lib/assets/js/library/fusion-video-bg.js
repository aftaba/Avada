/* global YT, Vimeo, fusionVideoBgVars, fusionGetConsent */
/**
...
if ( Number( fusionVideoBgVars.status_yt ) ) {
		...
		if ( 0 < $vimeos.length ) {
			...
			for ( i = 0; i < length; i++ ) {
				...
				if ( jQuery( 'html' ).hasClass( 'ua-ie-11' ) ) {
					jQuery( player ).parent().css( 'opacity', '1' );
				}
...
jQuery( window ).on( 'load fusion-element-render-fusion_builder_container', function( $, cid ) {
	...
	if ( 'undefined' !== typeof cid && Number( fusionVideoBgVars.status_yt ) ) {
